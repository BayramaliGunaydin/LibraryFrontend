...
function SingleUser(){
    ...
    const getlikes=()=>{
        fetch("http://localhost:9009/userlikes/"+localStorage.getItem("linkuserid"),{
            method:"GET",
            headers:{
              "Content-Type":"application/json",
              "Authorization":"Bearer "+localStorage.getItem("token")
    ...
    useEffect(()=>{
        fetch("http://localhost:9009/user/"+userid,{
            method:"GET",
            headers:{
              "Content-Type":"application/json",
              "Authorization":"Bearer "+localStorage.getItem("token")
    ...
    <Card className={classes.card}>
        ...
        <Avatar className={classes.root}>{user.pic?<img src={image}></img>:localStorage.getItem("username").charAt(0).toUpperCase()}</Avatar>
      ...
      <CardHeader className={classes.cardheader}
                ...
                <Userposts username={post.customuser.username} text={post.post} userid={post.customuser.id} userpic={post.customuser.pic} bookname={post.book.bookname}></Userposts>            
                     ...
                     <Card className={classes.userpostscard}><div> UserLikes{
                    error3 ? "error":
                    !isLoaded3?getlikes():
            ...
            {/* { <Card className={classes.userpostscard}><div> UserBooks{
                    error4 ? "error":
                    !isLoaded4?getbooks():
